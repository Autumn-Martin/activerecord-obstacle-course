...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'plucks all values from one column' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names = Item.select(:name).pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:order_items).where('item_id =?', item_8.id).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it 'returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item
      .joins(:order_items)
        .distinct
          .pluck(:name)
        # .distinct and .pluck need to be at the end
        # . distinct need to be before .pluck because it can't be called on the array that .pluck returns
    # ---------------------------------------------------------------
  ...
  it 'returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.includes(:items) # gives access to all items in one swoop, get entire order and include all of its items
    # ------------------------------------------------------
